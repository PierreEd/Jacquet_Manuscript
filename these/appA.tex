...
\chapter*{Appendix A: Error Signals}\label{app:App}
\addcontentsline{toc}{chapter}{Appendix A: Error Signals} % ensures it appears in the main TOC
...
\chapter*{Appendix B: Two-photon derivations}\label{app:App}
\addcontentsline{toc}{chapter}{Appendix B: Two-photon derivations} % ensures it appears in the main TOC
...
We then regroup the terms along common quadratures $\cos \omega_0 t$ and $\sin \omega_0 t$ such that we get
\begin{equation*}
  \begin{split}
...
\[\hat{S}(r, \theta) = \exp\left( \dfrac{r}{2} \int^{\infty}_{-\infty} \dfrac{d\Omega}{2\pi} \,\left[ e^{- i \theta(\Omega)} \hat{a}^{\vphantom{\dagger}}_+ \hat{a}^{\vphantom{\dagger}}_- - e^{ i \theta(\Omega)} \hat{a}^\dagger_+ \hat{a}^\dagger_- \right] \right)\]
where $r$ is the squeezing factor and $\theta(\Omega)$ the squeezing angle. Here we assumed the squeezing parameter to be frequency independent, but one can easily generalize to a frequency dependent squeezing parameter $r(\Omega)$. Using the sidebands annihilation operators defined previously, we can compute the action of the displacement and squeezing operators as
\begin{align*}
\hat{D}^\dagger(\alpha) \,\hat{a}^{\vphantom{\dagger}}_+ \,\hat{D}(\alpha) & = \hat{a}^{\vphantom{\dagger}}_+ + \alpha(\Omega) \,, \\
\hat{S}^\dagger(r, \theta) \,\hat{a}^{\vphantom{\dagger}}_+ \,\hat{S}(r, \theta) & = \hat{a}^{\vphantom{\dagger}}_+ \cosh r - e^{i \theta(\Omega)} \hat{a}^\dagger_- \sinh r \,.
\end{align*}
such that one can then compute the expectation values and variances of the quadrature operators for displaced squeezed states. We consider a intial vacuum state $|0\rangle$, and we displace it by a coherent amplitude $\alpha(\Omega) = \alpha \delta(\Omega)$ i.e. a carrier, monochromatic field of complex amplitude $\alpha$ sitting at frequency 0 (we factored out the $e^{-i\omega_0t} $ already) and squeeze it with parameters $r$ and $\theta(\Omega)$. The resulting state reads
\[|\psi\rangle = \hat{S}(r, \theta) \hat{D}(\alpha)  |0\rangle.\]
Applying both operators to the annihilation operators yields 
\begin{align*}
\hat{D}^\dagger(\alpha) \hat{S}^\dagger(r, \theta)  \,\hat{a}^{\vphantom{\dagger}}_+ \, \hat{S}(r, \theta) \hat{D}(\alpha) & = \left( \hat{a}^{\vphantom{\dagger}}_+ + \alpha \delta (\Omega) \right) \cosh r - e^{i \theta(\Omega)} \left( \hat{a}^\dagger_- + \alpha^* \delta (\Omega) \right) \sinh r \, \\
\hat{D}^\dagger(\alpha) \hat{S}^\dagger(r, \theta)  \,\hat{a}^{\dagger}_- \, \hat{S}(r, \theta) \hat{D}(\alpha) & = \left( \hat{a}^\dagger_- + \alpha^* \delta (\Omega) \right) \cosh r - e^{-i \theta(\Omega)} \left( \hat{a}^{\vphantom{\dagger}}_+ + \alpha \delta (\Omega) \right) \sinh r \,.
\end{align*}
such that the expectation valurs are computed as
\begin{align*}
\langle \hat{a}^{\vphantom{\dagger}}_{+} \rangle & = \delta(\Omega) (\alpha \cosh r - \alpha^* e^{i \theta(\Omega)} \sinh r) \,, \\
\langle \hat{a}^{\dagger}_{-} \rangle & = \delta(\Omega) (\alpha^* \cosh r - \alpha e^{-i \theta(\Omega)} \sinh r) \,.
\end{align*}
