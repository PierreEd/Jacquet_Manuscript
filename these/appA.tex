...
\paragraph{Derivative of the optimal readout angle at \texorpdfstring{$\Omega=\Omega_m$}{Omega = Omega\_m}.}
...
\label{eq:du}
\end{equation}
Combining Eqs.~\eqref{eq:dtheta_du} and \eqref{eq:du} yields the general expression
\begin{equation}
\frac{d\theta_{\rm opt}}{d\Omega}
=
\frac{
...
\end{equation}
Consequently \(u(\Omega_m)=0\), and Eq.~\eqref{eq:dtheta_general} simplifies to
\begin{equation}
...
\label{eq:dtheta_res_simplified}
\end{equation}
...
\label{eq:dReK_res}
\end{equation}
Inserting Eq.~\eqref{eq:dReK_res} into Eq.~\eqref{eq:dtheta_res_simplified} finally yields
\begin{equation}
\boxed{
\left.\frac{d\theta_{\rm opt}}{d\Omega}\right|_{\Omega_m}
=
...
|\mathcal K(\Omega_m)|
=
\left|\frac{\mathcal C^2}{2}\hbar\,\chi(\Omega_m)\right|
=
...
\chapter*{Appendix D: Cavity alignements}\label{app:AppD}
\addcontentsline{toc}{chapter}{Appendix D: Cavity alignments} % ensures it appears in the main TOC