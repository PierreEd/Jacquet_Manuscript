...
Using the linear optical system equations seen in Chap. I, A. Journeau and myself developed a program to numerically simulate the quadrature spectra of various optical systems. The idea, starting from standard vacuum fluctuations, is to propagate the quadrature operators through the optical system using the input-output relations of each optical element, taking into account losses and non-ideal efficiencies. Ideal squeezed states can be generated, a squeezer element, the finite OPO bdandwidth can be takeb into account by sending the squeezed state onto an symmetric resonant optical cavity element, frequency-dependence can be added by sending the squeezed state onto a single-ended detuned cavity element, and finally the optomechanical interaction can be modelled using a movable mirror element. The basic working principle is simply 
