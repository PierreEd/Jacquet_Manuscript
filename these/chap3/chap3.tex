...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\textwidth]{./chap2/fig/QSD_sqzFDS.pdf}
  \caption{Schematic representation of a frequency-dependent squeezed state generation using a detuned filter cavity. (a)-(b) Quantum sideband diagram of a frequency-dependent squeezed state generated by reflecting a frequency-independent squeezed state off a detuned filter cavity. (a) The resonant sidebands undergo a $\pi$-shift upon reflection, effectively rotating the squeezing ellipse by a frequency-dependent angle in (b). We show the transmission response of the filter cavity for illustration purposes only, as the squeezed state is reflected off the cavity. (c)-(d) Typical phase responses expected for both tabletop optomechanical experiments (c) and Virgo-LIGO type interferometers (d).}
  \label{fig:FDS_QSD}
\end{figure}
...
\subsection{Classical Description}\begin{figure}[!htbp]
  \centering
