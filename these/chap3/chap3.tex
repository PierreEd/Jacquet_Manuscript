...
\end{split}
\end{equation}
In this case, energy conservation i.e. $|\bar \alpha_L|^2 + |\bar \alpha_R'|^2 = |\bar \alpha_L'|^2 + |\bar \alpha_R|^2$ imposes that $2(\phi_t-\phi_r) = \pi$ such that we can chose $r_m = |r_m|$ and $t_m = i|t_m|$. We rewrite the the cavity fields by injecting the identities $\bar \alpha_L = - \bar \alpha_L' e^{2ikL_1}$ and $\bar \alpha_R' = - \bar \alpha_R e^{2ikL_2}$ such that we get the useful system 
\begin{equation}
\begin{split}
...
\begin{equation}
  ...
  \end{split}
\end{equation}
 and where we simplified the sines terms already equal to zero. 
...
\begin{equation}
  \dfrac{|\bar \alpha_R|^2}{|\bar \alpha_L'|^2} = \dfrac{P_R}{P_L} = \dfrac{1 + 2|r_m|\cos (2k L_1+2kx) + |r_m|^2}{1- |r_m|}. 
\end{equation}
with $P_{L,R} \propto |\bar \alpha_{L,R}|^2$. It then follows that the the power fraction leaking from the left and right mirrors, i.e. the resonant reflection and transmission coefficients $R(\Delta=0,x)$ and $T(\Delta = 0, x)$ are given by
\begin{equation}
  \begin{split}
...
To derive the position dependent cavity linewidth $\kappa(x)$ and finesse $\mathcal{F}(x)$, we once again resort to the Sankey et al. method \cite{sankey_strong_2010}. The total energy stored in the cavity is given by
\begin{equation}
  E = \dfrac{2(L_1+x)}{c} P_L + \dfrac{2(L_2 - x)}{c} P_R
\end{equation}
and the rate at which energy leaves the cavity is given by
\begin{equation}
  \partial_t E = - |t_1|^2 P_L + -|t_2|^2 P_R
\end{equation}
...
Using the same tools as in section II.2, we can derive the QLE of a membrane based optomechanical system. The membrane position now turns into an operator such that $\hat x \propto \hat c + \hat c^{\dagger} $ with $\hat c$ the mechanical annihilation operator as in the previous section. As seen above, the membrane position modifies the resonance frequencies of the two subcavities, such that they both depend on the membrane position as \(\omega_L(x)\) and \(\omega_R(x)\) but with inverse trend: when one cavity shortens and its FSR increases, the other lengthens and its FSR decreases. To first order, we can linearize the resonance frequencies as
\begin{equation}
...
\begin{align}
\hat H
&= \hbar (\omega_L + \tilde G_L x ) \hat a_L^\dagger \hat a_L^{\vphantom{\dagger}}
 + \hbar (\omega_{R-} - \tilde G_R x ) \hat a_{R-}^\dagger \hat a_{R-}^{\vphantom{\dagger}}
 + \hbar (\omega_{R+} - \tilde G_R x ) \hat a_{R+}^\dagger \hat a_{R+}^{\vphantom{\dagger}}
\tag*{(= $\hat H_\gamma$)}
\\[2mm]
...
\tag*{(= $\hat H_m$)}
\\[2mm]
&\quad + \hbar\tilde G_L \hat a_L^\dagger \hat a_L^{\vphantom{\dagger}} \, \delta \hat x
 - \hbar \tilde G_R (\hat a_{R+}^\dagger \hat a_{R+}^{\vphantom{\dagger}}
...
\begin{equation}
 \hat H_\gamma = \hbar  \tilde G_L x  \hat a_L^\dagger \hat a_L^{\vphantom{\dagger}}
 - \hbar  \tilde G_R \bigg(x+ \dfrac{\lambda}{4}\bigg)  \hat a_{R-}^\dagger \hat a_{R-}^{\vphantom{\dagger}}
 + \hbar \Bigg(\omega_{FSR} - \tilde G_R \bigg(x- \dfrac{\lambda}{4}\bigg) \Bigg) \hat a_{R+}^\dagger \hat a_{R+}^{\vphantom{\dagger}}\end{equation}
...
\begin{equation} 
  ...
  \mathbf{M}
  \begin{pmatrix}
    \hat a_L^{\vphantom{\dagger}} \\[6pt]
    \hat a_{R-}^{\vphantom{\dagger}} \\[6pt]
...
\begin{equation*}
  ...
  \begin{pmatrix}
    \tilde G_L x & -J & -J \\[6pt]
    -J & - \tilde G_R (x + \lambda/4) & 0 \\[6pt]
    -J & 0 & \omega_{FSR} - \tilde G_R (x - \lambda/4)
  \end{pmatrix}.
\end{equation*}
...
\end{equation}
where we omitted the tilde on the optomechanical coupling for ease of notation. 
