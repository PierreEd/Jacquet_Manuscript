...
This chapter will covers the theoretical aspects of squeezed light and its application to membrane optomechanics. We will first review the SQL concept in optomechanical measurements, and show how frequency-dependent squeezed light can be used to go beyond this limit. We will then present the theory of membrane based systems. 
\minitoc
\newpage
...
\begin{itemize}
  \item A cavity at resonance: $\Delta=0$.
...
\end{equation*}
Using these two relations, we can then express displacement fluctuations in terms of input amplitude and phase fluctuations, assuming the reflected field is a perfect probe of the mechanical resonator position fluctuations i.e. $\delta \hat q_{\mathrm{ref}}[\Omega] = \delta \hat q_x[\Omega]$ \cite{Jaekel1990}. This treatment is formally equivalent to considering the output phase as a statistical estimator of the position fluctuations being a stationary random process as done in quantum measurement theory \cite{clerk_introduction_2010}. We then write
\begin{equation}
...
\end{equation}
such that plugging back in Eq. \eqref{eq:total_displacement_spectrum} gives the SQL displacement spectrum as
\begin{equation}
  S_{xx}^{\rm SQL}[\Omega] = \hbar |\chi[\Omega]| \implies S_{xx}^{\rm SN}[\Omega] + S_{xx}^{\rm RPN}[\Omega] \geq \hbar |\chi[\Omega]|
\end{equation}
which is the fundamental limit to continuous position measurements with coherent light. We also note that for high-Q resonators, $\Omega_{SQL} \gg \Gamma_m$, so approximating the susceptibility by its real part holds over a relatively large frequency range but fails at the exact resonance where the susceptibility is purely imaginary.
\subsubsection{Thermal Noise}
Thermal noise is a major limitation in optomechanical experiments, as it can mask the quantum effects. The mechanical resonator is coupled to a thermal bath at temperature $T>0$, which drives the resonator into a thermal state with mean phonon occupation number $\bar n_{\mathrm{th}} \simeq k_B T / (\hbar \Omega_m)$ in the high temperature limit $k_B T \gg \hbar \Omega_m$. The position fluctuations induced by this thermal force are given by
\begin{equation}
...
\end{equation}
where we used the identity $\Im \chi[\Omega] = m\Gamma_m \Omega |\chi[\Omega]|^2$. At $T=0$ K, this reduces to the zero-point fluctuations spectrum $S_{xx}^{\mathrm{ZPF}}[\Omega] =\, m \Gamma_m \hbar \Omega_m  |\chi[\Omega]|^2 < S_{xx}^{\rm SQL}[\Omega]$, which is often neglected in the total displacement spectrum. However, at finite temperature, the thermal noise can be much larger than the SQL. Therefore, the total displacement spectrum now reads
\begin{equation}
...
\begin{itemize}
  ...
  \item A low temperature environment, such that the thermal noise $S_{xx}^{\mathrm{th}} \propto T$ is low and does not mask the quantum effects. This can be ensured by cryogenic cooling of the mechanical resonator, as well as using high-Q resonators to reduce the mechanical linewidth $\Gamma_m$.
\end{itemize}
...
\end{equation}
We then see that phase squeezing reduces the shot-noise contribution but increases the radiation-pressure noise contribution, while amplitude squeezing reduces the radiation-pressure noise contribution but increases the shot-noise contribution. The input cross-correlations being zero, this is completely equivalent to the coherent state with a rescaled input intensity $e^{\pm 2r} \bar{I}_\mathrm{in} $ (hidden in $\mathcal{C}$) for phase/amplitude squeezing respectively, as seen in Fig. \ref{fig:SQLVirgo}.(a) and Fig. \ref{fig:SQL_LKB}.(a). However, neither of these two configurations can reduce both contributions simultaneously, and therefore cannot improve the SQL limit as illustrated in Fig. \ref{fig:SQLVirgo}(b) and \ref{fig:SQL_LKB}(b) where only the displacement spectrum at $\theta=\pi/4$ goes below the SQL around the mechanical resonance frequency. \\ 

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\textwidth]{./chap3/fig/SQLVirgo.pdf}
  \caption{Displacement spectrum for a Virgo-LIGO type interferometer. (a) Spectra for coherent light (increasing powers), which doesn't allow to beat the SQL. (b) Spectra for the various squeezing angles at fixed power allowing to beat the SQL in the vicinity of the mechanical resonance only at $\theta= \pi/4$. Phase and amplitude squeezing are equivalent to rescaling the input power, and cannot beat the SQL. The grey area indicates the range over which sub-SQL sensitivity is achieved for a given squeezing factor. The grey line indicates optimal frequency-dependent squeezing angle.} 
  \label{fig:SQLVirgo}
\end{figure}
...
\begin{figure}[!htbp]
  ...
  \caption{Displacement spectrum for a tabletop optomechanical experiment. (a) As in the Virgo-LIGO type interferometer, spectra for coherent light (and increasing powers) which does not allow to beat the SQL. (b) Spectra for the various squeezing angles at fixed power allowing to beat the SQL around the mechanical resonance frequency only at $\theta= \pi/4$. The grey line indicates optimal frequency-dependent squeezing angle.}
  \label{fig:SQL_LKB}
\end{figure}
...
\end{equation}
Because $\tanh r$ tends to 1 as $r$ increases, the frequency range where the displacement spectrum is below the SQL decreases with increasing squeezing factor $r$: the larger the effect, the smaller the frequency band where it is effective. This is illustrated in Fig. \ref{fig:increasingR}. \\ 

\begin{figure}[t!]
  \centering
  \includegraphics[width=\textwidth]{./chap3/fig/FDS_increasingR.pdf}
  \caption{Displacement spectrum for a tabletop optomechanical experiment and for a Virgo-LIGO type interferometer, with frequency-independent squeezing angle $\theta=\pi/4$. As the squeezing factor $r$ increases, the frequency range over which the displacement spectrum goes below the SQL (dashed line) decreases.} 
  \label{fig:increasingR}
\end{figure}
...
\begin{equation}
  ...
  \end{split}
\end{equation}
As shown in Annex A, picking the squeezing angle as
\begin{equation}
  2 \theta[\Omega] = \arctan \left[\dfrac{2|\mathcal{K}[\Omega]|\cos \phi_m[\Omega]}{1 - |\mathcal{K}[\Omega]|^2}\right]
\end{equation}
...
\end{split}
\end{equation}
This broadband reduction of the displacement spectrum below the SQL is illustrated in Fig. \ref{fig:SQLVirgo}(b) and Fig. \ref{fig:SQL_LKB}(b), where the grey line indicates optimal frequency-dependent squeezing angle for both Virgo-LIGO and tabletop experiments. However, for a resonant optomechanical cavity i.e. $\Delta=0$, it is impossible to beat the SQL at the mechanical resonance, where the succeptibility is purely imaginary $\phi_m[\Omega_m] = \pi/2$. \\



\noindent \textbf{Convergence to VIRGO/LIGO notation:} We once again show that this general treatment matches the one used in the context of gravitational wave detectors. In the free mass regime, $\mathcal{K}[\Omega]$ is real, such that $\phi_m[\Omega]=\pi$. One can then rewrite the optimal squeezing angle as
\begin{equation}
  2 \theta[\Omega] = \arctan \Big[\dfrac{2\mathcal{K}[\Omega]}{1 - \mathcal{K}^2[\Omega]}\Big] = 2 \arctan \mathcal{K}[\Omega]
\end{equation}
where we used the identity $\arctan 2x/(1-x^2) = 2\arctan x \, \, (\text{mod}\, \pi)$, such that this comes down to the expression used in the context of gravitational wave detectors \cite{klmtv}. Furthermore, the mechanical frequency and damping rate will be significantly smaller than the $\hbar \mathcal{C}^2/m$ term such that using the free-mass succeptibility $\chi[\Omega] = -1/m \Omega^2$ boils down the SQL frequency to the well-known expression
\begin{equation}
...
\begin{figure}[t!]
  \centering
  \includegraphics[width=\textwidth]{./chap2/fig/QSD_sqzFDS_final2.pdf}
  \caption{Schematic representation of a frequency-dependent squeezed state generation using a detuned filter cavity. (a)-(b) Quantum sideband diagram of a frequency-dependent squeezed state generated by reflecting a frequency-independent squeezed state off a detuned filter cavity. (a) The resonant sidebands undergo a $\pi$-shift upon reflection, effectively rotating the squeezing ellipse by a frequency-dependent angle in (b). We show the transmission response of the filter cavity for illustration purposes only, as the squeezed state is reflected off the cavity. (c) shows the resulting noise spectra $S_{pp}$ and $S_{qq}$ i.e. PSD of the projected fluctuations. (d) Typical phase responses expected for both Virgo-LIGO type interferometers (left) and a tabletop optomechanics experiment (right).}
  \label{fig:FDS_QSD}
\end{figure}


To generate frequency-dependent squeezed states, one can use a detuned optical cavity called a filter cavity \cite{klmtv}. The principle is to reflect a frequency-independent squeezed state off a single-sided detuned cavity, such that only the sidebands resonant with the cavity will undergo a phase shift, effectively rotating the squeezing ellipse by a frequency-dependent angle as seen in Fig. \ref{fig:FDS_QSD}, resulting in the optimal quantum noise reduction (grey spectra in the figures above).  \\ 

In the tabletop optomechanical experiment context where the mechanical resonance frequency $\Omega_m$ is much larger than the mechanical linewidth $\Gamma_m$, the optimal squeezing angle varies rapidly around the mechanical resonance frequency, requiring a filter cavity with a bandwidth on the order of the mechanical linewidth $\kappa_{\rm FC} \sim \Gamma_m$. Since it needs to match the phase response of the optomechanical system dictated by the mechanical succeptibility, the filter cavity needs to be detuned by $|\Delta_{\rm FC}| = \Omega_m$ to provide the right rotation direction. We then want to derive the derivative of the optimal squeezing angle at the mechanical resonance frequency to relate it to the filter cavity bandwidth. We start from the optimal squeezing angle expression
\begin{equation*}
  \theta[\Omega] = \dfrac{1}{2} \arctan \left[\dfrac{2|\mathcal{K}[\Omega]|\cos \phi_m[\Omega]}{1 - |\mathcal{K}[\Omega]|^2}\right]
\end{equation*}
and we give the derivative at the mechanical resonance frequency $\Omega = \Omega_m$ derived in Annex A:
\begin{equation}
  \left.\frac{d\theta_{\rm opt}}{d\Omega}\right|_{\Omega_m}
=
-\frac{2|\mathcal K(\Omega_m)|}{\Gamma_m\left(1-|\mathcal K(\Omega_m)|^2\right)}
\end{equation}
As seen in Eq. \ref{eq:derivative_FC} for the detuned single-port cavity case, the derivative of the phase-shift picked up by the two-photon quadratures is given by 
\begin{equation*}
...
\begin{equation}
  ...
  \label{eq:derivative_FC2}
\end{equation}
Upon the single-port cavity assumption, this expression can be used to compute the required input mirror transmittivity of the filter cavity to achieve the desired bandwidth $\kappa_{\rm FC}$:
\begin{equation}
  T_{\rm in} = \dfrac{2L_{\rm FC} \kappa_{\rm FC}}{c} 
\end{equation}
where $L_{\rm FC}$ is the filter cavity length. Although this expression gives a first estimate for the required length and finesse necessary for such a task, a more accurate treatment taking into account losses and non-ideal escape efficiency is required to optimize the filter cavity design. This additionally motivates the use of three-mirror cavities as filter cavities to have more degrees of freedom to optimize the cavity parameters. Some properties of such cavities are detailed in the next subsection. \\

For a Virgo-LIGO type interferometer, where both the FC bandwidth and sideband frequencies of interest are comparable to the mechanical corner frequency $\kappa_{\rm FC}, \Omega \sim \Omega_m$, and where the free-mass approximation holds, the relative optimal squeezing angle varies slowly with frequency. Eventually, the assumptions we made to derive the tabletop optomechanical experiment filter cavity parameters break down, since both derivatives $\partial_\Omega \phi_+$ and $\partial_\Omega \phi_-$ contribute significantly to the optimal squeezing angle rotation rate. The filter cavity detuning is then set to $\Delta_{\rm FC} \sim - \Omega_{\rm SQL}$ to provide the right rotation direction around the SQL frequency. In this regime, the optimal filter cavity parameters were derived by C. Whittle \textit{et al.} \cite{whittle_optimal_2020}, leading to the expressions
\begin{equation}
\gamma_\text{FC}=\sqrt{\frac{\lambda^2+\sqrt{\lambda^4+\Omega_{\rm SQL}^4}}{2}},\qquad
|\Delta\omega_{\rm FC}|=\sqrt{\frac{-\lambda^2+\sqrt{\lambda^4+\Omega_{\rm SQL}^4}}{2}}
\end{equation}
where they define the coupler-limited bandwidth $\gamma_{\rm FC}$ and loss-limited bandwidth $\lambda$ as
\[\gamma_{\rm FC}=\frac{c\,T_{\rm in}}{4L_{\rm FC}},\qquad \lambda=\frac{c\,\mathcal{L}_{\rm rt}}{4L_{\rm FC}},\]
with $T_{\rm in}$ the input mirror power transmissivity, $\mathcal{L}_{\rm rt}$ the round-trip power loss, and $L_{\rm FC}$ the filter cavity length. \\ 

Regarding the experimental implementation of such filter cavities in the tabletop context, the main challenge lies in achieving the required narrow bandwidth for the squeezing ellipse twist, which can be as low as tens of mHz for high-Q mechanical resonators. This is discussed in more details in Chap. V, along with the locking strategies required to keep the filter cavity at a precise detuning. 


\subsection{Numerical simulations}
Using the linear optical system equations seen in Chap. I, we developed a Python program to numerically simulate the quadrature spectra of various optical systems, inspired from \cite{Oelker2016PhD,evans_realistic_2013,kwee_decoherence_2014,Isogai2016PhD}. The idea is to propagate the noises through a chain of optical systems using the transfer matrices derived in Chap. I, taking into account losses and non-ideal efficiencies. Taking an input noise matrix $ \mathbf{S}_\text{in}$ (potentially classical), transfer matrices $\mathbf{T_i}$ describing the linear optical elements as well as their loss contributions $\mathrm{\Lambda_i}$, the output noise matrices are then computed as
\begin{equation}
\begin{split}
  \mathbf{S}_{1}  = \mathbf{T}_{1} \, &\mathbf{S}_{\rm in} \, \mathbf{T_1}^T + \mathrm{\Lambda_1} \mathbf{1}\\
   &\vdots \\  
  \mathbf{S}_{i}  = \mathbf{T}_{i} \, &\mathbf{S}_{i-1} \, \mathbf{T_i}^T + \mathrm{\Lambda_i} \mathbf{1} \\
   &\vdots \\ 
  \mathbf{S}_{\rm out}  = \mathbf{T}_{N} \, &\mathbf{S}_{N} \, \mathbf{T_N}^T + \mathrm{\Lambda_N} \mathbf{1} \\
\end{split}
\end{equation}
with $n$ the number of optical elements. One can then simulate complex optical systems involving multiple cavities, squeezers, optomechanical elements, and detection stages. Inversely, one can use it to fit various experimental observations. \\

In practice, the \texttt{SetUp(lambda0, power)} class is created, specifying the wavelength and input optical power of the laser source. Ideal squeezed states can be generated by adding a squeezer element (\texttt{SetUp.add\_Squeezer(dB, angle, loss)}), the finite OPO bandwidth can be simulated by sending the squeezed state onto a symmetric resonant optical cavity element (\texttt{add\_Cavity(length, t1, t2, detuning, loss)}), frequency-dependence can be induced by sending the squeezer output onto a single-ended detuned cavity element (\texttt{add\_Cavity(...)}), optomechanical cavities can be added (\texttt{add\_IFO(length, m, Q, freq\_m, finesse)}), and finally detection projects the computed noise matrix onto a quadrature of interest, simulating a HD detection (\texttt{add\_ReadOut(angle, loss)}). Adding various loss elements (\texttt{add\_Loss(loss)}) at any point in the optical system is also possible. \\ 

This python simulation has been used to produce a number of plots of this manuscript, including Fig. \ref{fig:SQLVirgo}, \ref{fig:SQL_LKB}, and \ref{fig:increasingR}.


\section{Cavity Optomechanics with Membrane based systems } \label{sec:MATE}
\subsection{Classical Description}\begin{figure}[b!]
  \centering
  \includegraphics[width=\textwidth]{./chap3/fig/MATEcav.pdf}
  \caption{Schematic representation of a MIM/MATE system's classical fields under the closed cavity assumption. The colors match those of Fig. \ref{fig:models_MATE}.}
  \label{fig:Mate}
\end{figure}
...
\end{equation}
In the lossless case, we will assume the index of refraction $n$ is real, such that \(|r_m|^2 + |t_m|^2 = 1\). The right-moving mean field amplitudes in the left and right sub-cavities are denoted \(\bar \alpha_L\) and \(\bar \alpha_R\), while the left-moving mean field amplitudes are denoted \(\bar \alpha_L'\) and \(\bar \alpha_R'\) shown in Fig. \ref{fig:Mate}. The cavity fields are then related by 
\begin{equation}
\begin{split}
...
\end{split}
\end{equation}
In this case, energy conservation i.e. $|\bar \alpha_L|^2 + |\bar \alpha_R'|^2 = |\bar \alpha_L'|^2 + |\bar \alpha_R|^2$ imposes that $2(\phi_t-\phi_r) = \pi$ such that we can choose $r_m = |r_m|$ and $t_m = i|t_m|$. We rewrite the cavity fields by injecting the identities $\bar \alpha_L = - \bar \alpha_L' e^{2ikL_1}$ and $\bar \alpha_R' = - \bar \alpha_R e^{2ikL_2}$ such that we get the system 
\begin{equation}
\begin{split}
...
\begin{equation}
  ...
  \end{split}
\end{equation}
where we have simplified the sines terms already equal to zero. 
We will now consider the Membrane At The Edge (MATE) model where $L_1 \sim L \gg L_2 $ which implies $ \Delta L \sim L$. Solving for $\delta k$ reinjecting in the dispersion relation $\omega_c(x) = c k(x)$ leads to
\begin{equation}
...
\begin{equation}
  ...
  \end{split}
\end{equation}
which we plot in Fig. \ref{fig:models_MATE}.(d) for realistic parameters ($L=30$ mm, $r_m=0.7$). We see that the MATE configuration allows to reach significant quadratic optomechanical couplings at membrane positions where the linear coupling vanishes, as well as tunability of both couplings with the membrane position. \\

Although we don't exploit the $G^{(2)}$ coupling in this work, it adds a plethora of effects to study in future experiments, including quantum non-demolition measurements of the membrane phonon number \cite{Thompson2008Nature, jayich_dispersive_2008, clerk2010phononshot}, mechanical squeezing \cite{nunnenkamp2010quadratic_squeeze,vanner2011selective}, and preparation of non-classical mechanical states \cite{oconnell2010singlephonon,palomaki2013entangling}. \\
...
\begin{figure}[!htbp]
  ...
  \caption{Behaviour of the MATE resonance frequencies as a function of membrane reflectivity for a realistic system ($L=25$ mm, $T_1=100$ ppm, $T_2=20$ ppm) with different membrane reflectivities, ranging from 0.01 in red to 0.99 in blue. (a) Resonance frequencies as a function of membrane position. (b) Linear optomechanical coupling as a function of membrane position. The yellow and green dashed lines correspond to the two subcavities linear dispersive couplings (position independent). (c) Resonant outcoupling efficiencies as a function of membrane position. (d) Cavity finesse as a function of membrane position.}
  \label{fig:mate_res}
\end{figure}


\subsubsection{Cavity transmission and reflection}
From the system in Eq. \eqref{eq:cavity_fields_system}, and having derived just above the resonant cavity wavevectors $k$, we can compute the power ratio of the two sub-cavity fields as a function of $x$ when the MATE system is at resonance. This is
\begin{equation}
...
In the single-cavity \textit{effective mirror} approach, the membrane acts as a position-dependent mirror modifying the cavity resonance frequency as derived above. The back cavity then acts as an effective mirror with complex reflection coefficient given by 
\begin{equation}
\tilde r(x) = r_m + \dfrac{t_m^2 r_2 e^{2ik(L_2 - x)}}{1 - r_m r_2 e^{2ik(L_2 - x)}}
\end{equation}
such that the resulting cavity finesse is given by 
\begin{equation}
  \mathcal{F}(x) \sim \dfrac{2\pi }{|t_1|^2 + (1- |\tilde r(x)|^2)}
  \label{eq:effectiveF}
\end{equation} and the associated linewidth $\kappa(x) = \pi c / L \mathcal{F}(x)$. We compare both approaches in Fig. \ref{fig:models_F_comparison} for realistic parameters ($L=25$mm) and membrane reflectivities ranging from 0.01 (dim) and 0.99 (solid). Both models agree in the low reflectivity regime, while discrepancies arise at high reflectivities. Since the cavity is driven from the left port, we can sense that the effective mirror approach will accurately describe the cavity properties, as the Sankey \textit{et al.} method assumes energy storage in both subcavities without including the driving port asymmetry.

\begin{figure}[t!]
  \centering
  \includegraphics[width=\textwidth]{./chap3/fig/modelsF.pdf}
  \caption{Comparison of the two-cavity (solid lines) and effective mirror (dashed lines) approaches to derive the MATE cavity linewidth and finesse for various membrane reflectivities (tending to 1 as the line gets dimmmer). While both converge in the low reflectivity regime, discrepancies arise at high reflectivities, where they feature opposite trends. }
  \label{fig:models_F_comparison}
\end{figure}


\subsection{Quantum Description}
We now turn to the quantum description of the membrane-based optomechanical system. A question that naturally arises is how to best describe this three-mirror cavity quantum mechanically: should we consider two independent optical modes in each subcavity, coupled by photon tunneling through the membrane? Or should we consider the whole cavity as a single optical mode, whose resonance frequency is modified by the membrane position (and given above)? 
...
In V. Dumont's PhD work, quadratic points (where \(G^{(1)} = 0\) and \(G^{(2)} \neq 0\)) were the main focus of the study, in the high membrane reflectivity regime \cite{dumont_quantum_2022}. It was then sufficient to consider two optical modes coupled by photon tunneling through the membrane. \\ 

However, in our case, we focus on the sole dispersive coupling regime in the MATE configuration, and we additionally consider a low membrane reflectivity. The optimal point to do so is when the first long cavity is at resonance, and when the short one is anti-resonant. With a lowered reflectivity, the coupling between subcavity modes increases, leading to larger mode splittings at the avoided crossings, until the two subcavities are fully hybridized into new cavity modes spanning both subcavities. \\ 
...
\end{equation*}
We will only consider the photonic part of the Hamiltonian, in order to put it in matrix form such that we can diagonalize it and work in the basis of the new eigenmodes. Furthermore, we go to the frame rotating at frequency $\omega_0 = \omega_{R-} = \omega_L$ i.e. when the long cavity mode is degenerate with the left short cavity mode, such that the photonic Hamiltonian becomes
\begin{equation}
...
\begin{equation*}
  ...
  \end{pmatrix}.
\end{equation*}
One could then diagonalize this \(3 \times 3\) matrix to get the new eigenmodes of the system, and rewrite the optomechanical interaction in this new basis. In the limit where the membrane transmittivity is high such that \( |t_m| \sim 1\) and \( |r_m| \ll 1\), the tunneling rate \(J\) becomes much larger than both optomechanical couplings \(\tilde G_{L,R}\, x\) and the free spectral range \(\omega_{FSR}\). The cumbersome expression of the eigenmodes is not displayed here, but is equivalent to considering a system's eigenstate described by an annihilation operator \(\hat a \) with optomechanical coupling \( G^{(1)}(x)\) and decay rate \(\kappa(x)\) as derived in the previous section. The system's Hamiltonian can then be written as:
\begin{equation}
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\textwidth]{./chap3/fig/modelsCav.pdf}
  \caption{Comparison of (a) the resonance frequencies and (b) the linear optomechanical coupling derived from the two-cavity model and the single-cavity model, in the MATE configuration for membrane reflectivities of 0.01 and 0.99 ($L=25$mm). The solid lines correspond to the two-cavity model, while the dashed lines correspond to the single-cavity model.}
  \label{fig:mate_modes}
\end{figure}

\subsubsection{Single-Cavity Model}
...
\end{equation}
where \(\omega_c(x)\) is given by the expression derived above. This description then matches the two-cavity model in the limit of highly-reflective membranes as seen in Fig. \ref{fig:mate_modes}, as well as in the limit of low-reflectivity membranes where the subcavities are fully hybridized. 

\subsubsection{Comparison}
Since we are interested in the dispersive coupling regime in the MATE configuration with a low-reflectivity membrane, such that we will operate the system where the linear dispersive coupling is dominant over quadratic dispersive coupling and dissipative coupling, we need to check which model is best suited to describe the system. \\

Obviously the two-cavity model breaks down in the limit of low-reflectivity membranes where the subcavities are fully hybridized, and the single-cavity model is then more appropriate. In the opposite limit of highly reflective membranes, both models converge to the same description as seen above. Regarding the radiation pressure force acting on the membrane, in the two-cavity model, the radiation pressure force is given by the sum of the forces exerted by each subcavity mode as \( \hat F_{rp} = - \hbar G_L \hat a_L^\dagger \hat a_L + \hbar G_R (\hat a_{R+}^\dagger \hat a_{R+} + \hat a_{R-}^\dagger \hat a_{R-})\). In the single-cavity model, the radiation pressure force is given by \( \hat F_{rp} = - \hbar \partial \omega_c(x)/\partial x \, \hat a^\dagger \hat a\). In the limit of highly reflective membranes, the two-cavity model radiation pressure force is then more appropriate since the optical mode is split in two subcavity modes, each exerting a force on the membrane. In the opposite limit of low-reflectivity membranes where the subcavities are fully hybridized and where we focus on the dispersive coupling regime, it would genuinely be of no interest when studying radiation pressure effects, and a relevant description of the radiation pressure force is tricky to derive (because there are actually photons in both subcavities, but the model breaks down). In our middle ground case of moderately reflective membranes, we will assume the optical mode is mostly localized in the long cavity mode such that the single-cavity description is valid, and that the radiation pressure force is given by the derivative of the cavity resonance frequency with respect to the membrane position as seen in the textbook case of a single mirror cavity. The same QLEs as in the previous chapter can then be derived and used in our case. 
