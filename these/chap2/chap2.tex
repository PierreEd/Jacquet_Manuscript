...
\begin{equation}
    ...
    =
    \mathbf S_{\mathrm{cl}}[\Omega]
    +
    \mathbf S_{\mathrm{qu}}[\Omega].
...
\begin{itemize}
  ...
  \item A type I squeezed state: the correlation between the two sidebands is quantum. 
\end{itemize}
...
\subsubsection*{Example II: Type I Squeezed state}
We now consider a type I squeezed state, where the two sidebands at $\pm \Omega$ are correlated by a $\chi^{(2)}$ nonlinear process. The classical contribution to the covariance matrix is zero, $\mathbf S_{\mathrm{cl}}[\Omega] = \mathbf{0}$, while the quantum contribution $\mathbf S_{\mathrm{qu}}[\Omega]$ is given by Eq. \eqref{II.xx4}.  \\ 
