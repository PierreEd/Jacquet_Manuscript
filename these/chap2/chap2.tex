...
Although Example I below illustrates the Mode Cleaner working principle, we will focus on the reflected quadratures regarding the detuned case, as optomechanical cavities and the quantum filter studied in this thesis are operated in reflection, in the single-port cavity configuration i.e. $\kappa_2 = 0$ and $\kappa \sim \kappa_1$ and where we assume negligible losses. We can then approximate the reflected quadrature transfer matrices in Eq.\ref{eq:cav_transf} as
\begin{equation}
...
\end{equation*}
Here, the cavity acts as an ideal phase retarder, rotating the quadratures by an angle $- \bar \phi$ and adding an overall phase shift $\delta \phi$ that vanishes when computing noise spectra. Explicitely, the two-photon average phase shift and its derivative are given by 
\begin{equation}
