...
\begin{equation}
  \mathbf{S}^{\mathrm{out}}[\Omega] = \mathbf{T} \mathbf{S}^{\mathrm{in}}\mathbf{T}^{\dagger} + \mathbf{L} \mathbf{L}^{\dagger} 
\end{equation}
...
such that
\begin{equation}
  \mathbf{S}^{\mathrm{out}}_{\phi}[\Omega] = \mathbf{T}^{\vphantom{\dagger}}_\phi \mathbf{S}^{\mathrm{in}}_{\phi}\mathbf{T}_\phi^{\dagger} + \mathbf{L}^{\vphantom{\dagger}}_\phi \mathbf{L}_\phi^{\dagger}
\end{equation}
...
=  \mathbf{T} \, \mathbf{\delta \hat{u}} \quad \text{with} \quad
...
\begin{equation}
  \mathbf S^{\text{sqz}}[\Omega] = \mathbf{T}\mathbf{S}^{\text{vac}} \mathbf{T}^{\dagger} = \mathbf{T}\mathbf{T}^{\dagger}.
\end{equation}
Explicitly, this yields
\begin{equation}
  \begin{split}
\mathbf{S}^{\text{sqz}}[\Omega] &  =
\begin{pmatrix}
...
\begin{equation}
  \begin{split}
  \mathbf{S}^{\text{sqz}}_{\phi}[\Omega] & = \mathbf{R}(\phi) \, \mathbf{S}^{\text{sqz}}[\Omega] \, \mathbf{R}(-\phi) \\
  & =  \mathbf{R}(\phi-\theta ) \begin{pmatrix}
e^{-2r} & 0 \\[6pt]
0 & e^{2r}
\end{pmatrix}
\mathbf{R}(\theta-\phi).
...
\begin{equation*}
\mathbf{S}^{\text{sqz}}_{\phi=\theta}[\Omega] =
\begin{pmatrix}
e^{-2r} & 0 \\[6pt]
0 & e^{2r}
\end{pmatrix}.
...
\noindent \textbf{Two fields at different frequencies: } Finally, let's consider the case where the signal and LO fields are at slightly different wavelengths/frequencies, such that $\hat a$ is at frequency $\omega_0$ and $\hat a_{\mathrm{LO}}$ at frequency $\omega_0 + \Delta \omega$. The total field impinging on the photodiode is then written as $\hat a_{\mathrm{tot}} = \hat a + \hat a_{\mathrm{LO}} e^{-i \Delta\omega t}$ since our operators are defined in a frame rotating at $\omega_0$. The mean photocurrent is then given by
\begin{equation}
  \begin{split}
    \bar I &= e \, \bigg(|\bar \alpha|^2 + |\bar \alpha_{\mathrm{LO}}|^2 + 2 |\bar \alpha| |\bar \alpha_{\mathrm{LO}}| \cos (\Delta\omega t + \phi_{\mathrm{LO}}) \bigg)
  \end{split}
\end{equation}
where we see that the interference term now oscillates at the beat frequency $\Delta \omega$. Demodulating the photocurrent at a frequency $\Delta \tilde \omega \sim \Delta \omega$, phase $\tilde \phi$, and low pass filtering the photocurrent then gives 
\begin{equation}
\bar I_{\rm demod} \propto   \cos ((\Delta \omega - \Delta \tilde \omega)t + \phi - \tilde \phi).
\end{equation}
