...
This chapter establishes the theoretical framework for describing coherent and squeezed optical fields, their interaction with various optical cavities, and the measurement of their quantum properties. It begins with a review of optics and field quantization, then describes CW coherent and squeezed states (gaussian), cavity interactions, and detection schemes used in the lab to characterize such optical states. 
\minitoc
\newpage
\section{Optics}
...
Beyond the fundamental Gaussian mode, more general solutions of the paraxial equation can be constructed. In Cartesian coordinates for example, these are the Hermite--Gaussian modes $\mathrm{TEM}_{mn}$, given by
\begin{multline}
...
\caption{ Gaussian beam characteristics: (a) Intensity profile of the fundamental Gaussian mode at different positions along the propagation axis $z$. The various quantities introduced in the text are indicated. (b) Transverse intensity profiles of the first few Hermite--Gaussian modes $\mathrm{TEM}_{mn}$. These modes form an orthonormal basis to describe the spatial structure of paraxial beams: any realistic beam can be decomposed as a superposition of these modes. } 
\end{figure}

\subsection{Quantum Description}
\subsubsection{Quantised Electromagnetic Field }
% \the\textwidth
We will consider both cases of a finite quantization volume $V$ and an infinite volume: when dealing with cavity modes, we will use the finite volume description, while for propagating modes carrying sideband correlations we will use the infinite volume description, under the two-photon formalism prescription. Both descriptions are linked through the input-output theory introduced later on. \\

We first consider the quantized electromagnetic field in a volume $V$. The electric field operator can be written as \cite{knightoptics}
\begin{equation}
...
\end{equation}
where   $\mathcal{E}_\ell = \sqrt{\hbar \omega_\ell / 2 \varepsilon_0 V}$ is the \textit{discrete} field amplitude per photon in mode $\ell$, $\hbar$ is the reduced Planck constant, $\omega_\ell$ is the angular frequency of mode $\ell$, and $\varepsilon_0$ is the vacuum permittivity. The spatial mode functions $\mathbf{f}_{\ell}(\mathbf{r})$ form an orthonormal basis in $V$ according to  
\begin{equation*}
...
In the limit of an infinite quantization volume $V \to \infty$, the discrete mode index $\ell$ becomes a continuous variable i.e. the mode spacing becomes infinitesimal. As detailed in Appendix A, we now need to consider a continuum of modes with annihilation operators $\hat a[\omega]$ labeled by their angular frequency $\omega$. To come down to the two-photon formalism, we make the following assumptions:
\begin{itemize}
    \item We consider frequencies $\omega = \omega_0 \pm \Omega$ centered around a carrier frequency $\omega_0$, with $\Omega  \in [-2 \pi B,+2 \pi B]$ and $ |\Omega| \ll \omega_0$, where $B$ is the bandwidth. This is valid since the bandwidth $B$, generally up to tens of GHz, is small compared to $\omega_0/2\pi$ which is hundreds of THz for optical frequencies. 
...
\begin{equation}
  \begin{split}
   \hat{E}(t) =  \tilde{\mathcal{E}}_0 \bigg[ \cos ( \omega_0 \, t - \frac{\pi}{2} )   \int^{\infty}_{-\infty} \dfrac{d\Omega}{2\pi} \, \ & (\hat{a}_+^{\vphantom{\dagger}} + \, \hat{a}_-^{\dagger})\,e^{-i\Omega t}   \\ 
    +  \sin ( \omega_0 \, t - \frac{\pi}{2} )   \int^{\infty}_{-\infty} \dfrac{d\Omega}{2\pi} \, & i(\hat{a}_-^{\dagger} - \hat{a}_+^{\vphantom{\dagger}}  \, )\,e^{-i\Omega t} \bigg] 
  \end{split}
\end{equation}
where we defined the sideband annihilation operators as $\hat{a}_+[\Omega] \equiv \hat{a}[\omega_0 + \Omega]$ and $\hat{a}_-[\Omega] \equiv \hat{a}[\omega_0 - \Omega]$. The \textit{continuous} field amplitude per photon at the carrier frequency is given by $\tilde{\mathcal{E}}_0 = \sqrt{\hbar \omega_0/2 \varepsilon_0 c A}$, where $A$ is the effective cross-sectional area of the beam and $c$ the speed of light in vacuum. The explicit relationship between the discrete mode operator $\hat a_\ell^{\vphantom{\dagger}}$ and the continuous mode operator $\hat a[\omega]$ (and thus their hermitian conjugate) is given in Appendix A. \\ 

\noindent \textbf{Note:} Although the electric field operator is written in the Heisenberg picture, the annihilation and creation operators $\hat a[\omega]$ and $\hat a[\omega]^\dagger$ are Schrödinger-like operators, i.e. time-independent operators. This is because we already factored out the time dependence $e^{-i\omega t}$ associated to each frequency mode when writing our annihilation/creation operators. As seen later on, the time dependence of the field operator defined through the Fourier transform arises from the superposition of many frequency modes, leading to beating at frequency $\Omega$. This is the heart of the two-photon formalism, where a time-dependent field Heisenberg-like operator is built from the superposition of Schrödinger-like annihilation/creation operators at different frequencies. In the literature, the Schrödinger-like operators are sometimes written as $\hat a_\omega$ to draw a parallel between the discrete mode case $\hat a_\ell$, but we will stick  to the $\hat a[\omega]$ notation to avoid confusion with time-dependent operators. \\ 
...
As demonstrated in Appendix A, the continuous annihilation and creation operators satisfy the following commutation relations: 
\[
...
\end{equation}
where we defined the field vector $\mathbf{\hat{a}}$ and the transfer matrix $\mathbf \Gamma$, later used to switch from \textit{one-photon} to \textit{two-photon} description of optical elements. Considering the electric field operator defined above, we can safely extend the integration limits to infinity, such that it can be recasted as 
\begin{equation}
\hat{E}(t) = \, \tilde{\mathcal{E}}_0 \,\left[ \cos ( \omega_0 \, t - \frac{\pi}{2} ) \int_  {-\infty}^{\infty} \dfrac{d\Omega}{2\pi}  \,\hat a_1[\Omega] \,e^{-i\Omega t} + \, \sin ( \omega_0 \, t - \frac{\pi}{2} )  \int_ {-\infty}^{\infty} \dfrac{d\Omega}{2\pi}  \,\hat a_2[\Omega] \,e^{-i\Omega t} \right].
\end{equation}
...
We now come to the aforementioned construction of a time-dependent field operator from the superposition of many frequency modes. This is done through the Fourier transform defined as
\begin{equation}
  \begin{split}
...
\begin{equation}
[\mathbf{\hat{a}}, \mathbf{\hat{a}}^{\dagger}] = \mathbf{\hat \sigma_z} \times 
\begin{cases} 
\delta (t-t')  \\
2\pi \delta (\Omega - \Omega') .
\end{cases}
\end{equation}
with $\mathbf{\hat \sigma_z}$ the Pauli Z matrix, and where it is implicit that we evaluate both at different frequencies and times respectively.  
...
Let's now consider a quantum state $| \psi \rangle$ living in this continuous mode space. We can always linearize the field operators around their mean value, which is particularly useful when dealing with intense fields featuring small quantum fluctuations around a large classical amplitude. This is the case for coherent and squeezed states, which are introduced right below. The annihilation operator is then be decomposed as
\begin{equation}
...
\label{eq:Sa}
\end{equation}
where \(\{\hat{A},\hat{B}\} = \hat{A}\hat{B} + \hat{B}\hat{A}\) denotes the anticommutator, implicitely evaluated at frequencies \(\Omega\) and \(\Omega^{'}\), and integrated over \(\Omega'\). The diagonal elements of the Noise Spectral Density matrix correspond to the power spectral densities of the quadrature fluctuations, while the off-diagonal elements represent the cross-spectral densities between both quadratures. The Noise Spectral Density matrix is a Hermitian matrix, reflecting the physical properties of the quantum state. We will particularly focus on the amplitude and phase quadrature Noise Spectral Density matrix, obtained by setting \(\phi = 0\), and we will denote it as \(\mathbf{S}[\Omega]\equiv \mathbf{S}_{\phi=0}[\Omega]\). The subscripts will then denote whether we refer to the transmitted or reflected fields of an optical cavity, the output spectrum of a squeezer, etc. 
For completeness we introduce the \textit{single-sided} Noise Spectral Density matrix, defined as
\begin{equation}
  \mathbf{\bar S}_{\phi}[\Omega]= \dfrac{1}{2} (\mathbf{S}_{\phi}[\Omega]+ \mathbf{S}_{\phi}[-\Omega]).
\end{equation}
\noindent
While $\mathbf{S}_{\phi} [\Omega]$ is the fundamental two-sided Noise Spectral Density defined from the symmetrized correlation functions, the one-sided spectrum $\mathbf{\bar S}_{\phi}[\Omega]$ is the quantity directly accessible in experiments and ensures a direct correspondence between spectral integration and the variance of the quadrature. A generalized version of the Heisenberg uncertainty relation can be expressed in terms of the Noise Spectral Density matrix as
\begin{equation}
...
For the vacuum state \(|0\rangle\), we derive the Noise Spectral Density matrix using the commutation relations and the fact that \(\langle \delta \hat{a}  \rangle = 0\) (see Annex A). The calculation yields
\begin{equation}
...
Gaussian states can then be pictured in a 2D space, where the two axes correspond to the $\hat a_1$ and $\hat a_2$ quadratures. In the case where the mean phase is zero, these quadratures correspond to the amplitude and phase quadratures $\hat p$ and $\hat q$. The quantum state can then be represented as a 2D Gaussian distribution centered around the mean values of the quadratures, with the shape and orientation of the distribution set by the Noise Spectral Density matrix elements. The uncertainties in the quadratures - the diagonal elements - are represented by the widths of the Gaussian distribution along each axis, while correlations between the quadratures - the off-diagonal elements - are represented by the tilt of the distribution. This graphical representation provides an intuitive way to visualize and understand the properties of Gaussian quantum states, such as thermal, coherent and squeezed states, in terms of their quadrature fluctuations and correlations. Examples of such representations are shown in Fig. \ref{fig:quantum_states}. 
...
(c) Squeezed vacuum state: an elliptical Gaussian centered at the origin, with reduced noise along a rotated quadrature and increased noise in the orthogonal direction. 
(d) Bright squeezed state: an ellipse shifted away from the origin, combining anisotropic fluctuations and a non-zero mean amplitude. The displacement angle $\varphi$ and squeezing angle $\theta$ are independent. } 
...
\end{pmatrix}
\label{eq:I.quad_unmod}
\end{equation}
such that the components reduce to $2\mathrm{Re} (\alpha)$ and  $2\mathrm{Im} (\alpha)$ if $\phi=0$, and to $2|\alpha|$ and $0$ if $\phi = \bar{\varphi}$, or equivalently if we set $\bar{\varphi} = \phi  = 0$ as mentioned earlier (such that $\mathbf{\hat u}$ corresponds to the amplitude and phase quadratures). We also notice the delta function at $\Omega = 0$, indicating that the coherent state has a non-zero mean field at the carrier frequency only. \\
...
\begin{equation}
  \begin{split}
\mathbf{S}_{\text{sqz}}[\Omega] &  = \mathbf{R}(-\theta) e^{-2r \hat \sigma_z} \mathbf{R}(\theta)\\
& = \begin{pmatrix}
...
\begin{equation}
  \alpha(t) = \bar{\alpha} \left(1 + \epsilon_a \cos(\Omega_{\text{mod}} t)\right), \quad \quad (\epsilon_a \in \mathbb{R})
\end{equation}
with $|\epsilon_a| \ll 1$, the field amplitude modulation depth. While the DC term lives at frequency $\omega_0$, the modulation introduces sidebands at frequencies $\omega_0 \pm \Omega_{\text{mod}}$ in phase with the carrier, seen by expanding the cosine:
\begin{equation}
...
\begin{equation}
\alpha(t) = \bar{\alpha} \, e^{i \epsilon_{\phi} \cos(\Omega_{\mathrm{mod}} t)}, \quad \quad (\epsilon_{\phi} \in \mathbb{R})
\label{eq:PM_def}
\end{equation}
...
\label{eq:PM_expand}
\end{equation}
While the carrier term lives at frequency $\omega_0$, the modulation introduces sidebands at $\omega_0 \pm \Omega_{\mathrm{mod}}$, both shifted by $\pi/2$ relative to the carrier.\\ 


In both cases, amplitude or phase modulations, the field contains a carrier at frequency $\omega_0$ and two sidebands at $\omega_0 \pm \Omega_{\mathrm{mod}}$. Amplitude modulation results in sidebands that are in phase with the carrier, while phase modulation produces sidebands with a $\pm \pi/2$ phase-shift relative to the carrier. More generally, an arbitrary weak modulation process can be written as
\begin{equation}
    \alpha(t) = \bar{\alpha}\left(1+\varepsilon(t)\right),
\end{equation}
where $\varepsilon(t) = \epsilon_a(t) + i\epsilon_{\phi}(t) $ describes weak fluctuations of the complex field amplitude. The real and imaginary parts of $\varepsilon(t)$ respectively correspond to amplitude and phase modulations. 
Computing the Fourier transforms for amplitude and phase modulations yield
\begin{equation}
  \begin{split}
...
It then follows that the linearized amplitude-phase quadrature operators can be expressed as
\begin{equation}
\mathbf{\hat u}_{\bar{\varphi}}(t)
=
\underbrace{
2|\bar{\alpha}|
\begin{pmatrix}
...
0
\end{pmatrix}
}_{\text{carrier / DC term}}
+
\underbrace{
2|\bar{\alpha}|
\begin{pmatrix}
\mathrm{Re}\!\left(\varepsilon(t)\right)\\
\mathrm{Im}\!\left(\varepsilon(t)\right)
\end{pmatrix}
}_{\text{classical modulation}}
+
\underbrace{
\begin{pmatrix}
\delta \hat p(t)\\
\delta \hat q(t)
\end{pmatrix}
}_{\text{quantum fluctuations}}.
\end{equation}
where we identify a carrier term, a time-dependent  yet deterministic coherent displacement associated with classical modulations, and an operator fluctuation contribution. Assuming vanishing mean operator fluctuations,
$\langle \delta \hat p\rangle=\langle \delta \hat q\rangle=0$, the covariance matrix can then be split into a coherent modulation contribution and an operator fluctuation contribution, as the DC term does not contribute to the fluctuations:
\begin{equation}
    \mathbf S_{\bar{\varphi}}[\Omega]
    =
    \mathbf S_{\bar{\varphi}}^{\mathrm{cl}}[\Omega]
    +
    \mathbf S_{\bar{\varphi}}^{\mathrm{q}}[\Omega].
\label{eq:modulated_field_fluctuationn}
\end{equation}
The coherent contribution reads
\begin{equation}
    \mathbf S_{\bar{\varphi}}^{\mathrm{cl}}[\Omega]
    =
    4|\bar{\alpha}|^2
    \begin{pmatrix}
        \left|\mathrm{Re}\!\left(\varepsilon[\Omega]\right)\right|^2
        &
        \mathrm{Re}\!\left(\varepsilon[\Omega]\right)
        \mathrm{Im}\!\left(\varepsilon[\Omega]\right)^*
        \\[2mm]
        \mathrm{Im}\!\left(\varepsilon[\Omega]\right)
        \mathrm{Re}\!\left(\varepsilon[\Omega]\right)^*
        &
        \left|\mathrm{Im}\!\left(\varepsilon[\Omega]\right)\right|^2
    \end{pmatrix},
\end{equation}
while the operator contribution $\mathbf S_{\bar{\varphi}}^{\mathrm{q}}[\Omega]$ is defined as in Eq. (\ref{eq:Sa}). 

\newpage
\subsection{Quantum Sideband Diagram }

We now have all the tools to graphically represent continuous wave (CW) quantum states of light in the frequency domain. The so-called \emph{quantum sideband diagram} is a useful representation to visualize optical quantum states time evolution, interferences and quantum noises, especially when dealing with modulated coherent or squeezed fields \cite{chelkowski2007,chua2013}. The central idea is that each sideband mode $\omega_0 + \Omega$ out of the frequency continuum can be treated as a quantum harmonic oscillator, featuring some level of correlation with its conjugate sideband mode at $\omega_0 - \Omega$, and represented in its own phase space with quadratures defined as $\hat{p}' = \hat{a}_+ + \hat{a}^\dagger_+$ and $ \hat{q}' = i(\hat{a}^\dagger_+ - \hat{a}_+)$, different from the two-photon quadratures defined in Eq. (\ref{II.1}), i.e. $(\hat{p}', \hat{q}') \neq (\hat{p}, \hat{q})$. While the first pair corresponds to tracing out a quantum state in a given sideband mode, and looking at the (hardly physical) reduced state in phase space, the two-photon quadratures capture the fluctuation correlations between the two sidebands, and are the relevant quadratures when measuring the field with a photodetector \cite{caves_new_1985,schumaker_new_1985}. In what follows, we will, however, label the axes with $(\hat{p}, \hat{q})$ in all quantum sideband diagrams for simplicity.\\  


In this representation, we plot the carrier frequency \(\omega_0\) at the center, and the sidebands at frequencies \(\omega_0 \pm \Omega\), such that once in the frame rotating at \(\omega_0\), the carrier field is a DC term with an fixed amplitude and phase, while the conjugate sidebands at \(\pm \Omega\) rotate counter-clockwise and clockwise, respectively. Each sideband mode is in a quantum state: the vacuum state, a coherent state, a thermal state, etc. As seen in Eq. (\ref{eq:modulated_field_fluctuationn}), the covariance matrix reads $ \mathbf S_{\bar{\varphi}}[\Omega]=  \mathbf S_{\bar{\varphi}}^{\mathrm{cl}}[\Omega] + \mathbf S_{\bar{\varphi}}^{\mathrm{q}}[\Omega]
$, where we identify both a deterministic contribution from classical modulations, as well as a quantum contribution, such that the noise seen at a given frequency $\Omega$ is the sum of these two. Importantly, correlations between conjugate sideband modes can then originate from classical modulation processes described by $\mathbf S_{\bar{\varphi}}^{\mathrm{cl}}[\Omega]$, or from a quantum process such as squeezing described by $\mathbf S_{\bar{\varphi}}^{\mathrm{q}}[\Omega]$. The first term describes correlations of the coherent amplitudes, while the second term describes fluctuation correlations. We will graphically represent the correlated contributions as arrows, and the uncorrelated contributions as isotropic Gaussian distributions regardless of the correlation type, as to graphically explain the origin of the elliptical shapes in phase space. \\  

We will now discuss two instructive cases of quantum states of light and their representations:
\begin{itemize}
  \item A phase-modulated coherent state: the correlation between the two sidebands is classical.
  \item A squeezed state: the correlation between the two sidebands is quantum. 
\end{itemize}


\noindent \textbf{Note:} In the time domain, the beating of two conjugate sidebands at $\pm \Omega$ with the carrier field results in a signal oscillating at frequency $\Omega$, which can be averaged over time to extract the power spectral density (PSD) of the signal at frequency $\Omega$. In the quantum sideband diagram, this beating process can be represented by the vector sum of the correlated contributions, while the quantum noise represented at the tip of the vector sum is to be computed explicitely.

\subsubsection{Example I: Phase-modulated coherent state}

For a coherent state, the quantum fluctuation contribution is the vacuum noise i.e. $\mathbf S_{\bar{\varphi}}^{\mathrm{q}}[\Omega] = \mathbf{1}$, which is isotropic and uncorrelated across all frequencies, that we represent by a isotropic Gaussian distribution. We show the quantum sideband diagram of a phase-modulated coherent state in Fig. \ref{fig:QSD_mod_phase}.(a), where the two sidebands at $\pm \Omega_{\mathrm{mod}}$ are created with a $\pi/2$ phase shift relative to the carrier field (Eq. (\ref{eq:PM_expand})). A classical (noiseless) phase modulation process therefore displaces the vacuum state at $\pm \Omega_{\mathrm{mod}}$ in the $q$ quadrature, while the other sideband modes remain in the vacuum state. Upon time evolution, the states at $\pm \Omega_{\mathrm{mod}}$ rotate in opposite directions, and their vector sum yields a signal oscillating at frequency $\Omega_{\mathrm{mod}}$ in the $(q,\Omega)$ plane, as seen in Fig. \ref{fig:QSD_mod_phase}(b). Averaging this signal over time (Fig. \ref{fig:QSD_mod_phase}(c)), one can then measure the signal PSD (Fig. \ref{fig:QSD_mod_phase}(d)), which consists of a coherent spectral line at $\Omega_{\mathrm{mod}}$ on top of a flat vacuum noise background. \\ 

In this example, the correlation between the two sidebands is purely classical: the two sidebands are displaced in phase space with a known phase relation, such that their time evolution is deterministic. The fluctuations of two conjugate sidebands however, remain totally uncorrelated. A classically modulated coherent state can therefore never exhibit noise below vacuum level. 


\begin{figure}[t!]
\centering
\vspace{0.5cm}
\includegraphics[width=\textwidth]{./chap2/fig/mod1.pdf}
\caption{Quantum sideband diagram of a phase-modulated coherent state. (a) The two sidebands at $\pm \Omega_{\mathrm{mod}}$ are displaced in the $q$ quadrature with a $\pi/2$ phase shift relative to the carrier field, while the other sideband modes remain in the vacuum state. (b) Upon time evolution, the states at $\pm \Omega_{\mathrm{mod}}$ rotate in opposite directions, and their vectorial sum yields a signal oscillating at frequency $\Omega_{\mathrm{mod}}$ in the $(q,\Omega)$ plane. (c) Averaging this signal over time, one can then measure the signal PSD (d), which consists of a coherent spectral line at $\Omega_{\mathrm{mod}}$ on top of a flat vacuum noise background. A similar representation can be drawn for an amplitude-modulated coherent state, where the two sidebands are displaced in the $p$ quadrature instead of the $q$ quadrature.}
\label{fig:QSD_mod_phase}
\end{figure}

\subsubsection*{Example II: Squeezed state}
We now consider a (type I) squeezed state, where the two sidebands at $\pm \Omega$ are correlated by a $\chi^{(2)}$ nonlinear process. The classical contribution to the covariance matrix is zero, $\mathbf S_{\bar{\varphi}}^{\mathrm{cl}}[\Omega] = \mathbf{0}$, while the quantum contribution $\mathbf S_{\bar{\varphi}}^{\mathrm{q}}[\Omega]$ is given by Eq. \eqref{II.xx4}.  \\ 


We focus on the graphical representation of such states. We reproduce the quadrature statistics of squeezed-state fluctuations using an
equivalent \textit{vector-sum of correlated random variables} picture, mimicking the interference of the two symmetric sidebands entering the quadrature observables. Let $\mathbf v=(\cos\theta,\sin\theta)^T$ be the squeezed quadrature direction,
and $\mathbf u=(-\sin\theta,\cos\theta)^T$ the associated anti-squeezed direction.
We draw a pair of correlated random sideband contributions whose vector sum is
constrained along $\mathbf u$,
...
\end{pmatrix}.
\end{split}
\label{eq:covariance_squeezed_state}
\end{equation}
This is strictly the same noise matrix obtained in the two-photon formalism, where the diagonal terms give the variances and the off-diagonal terms encode the correlations/anticorrelations between quadratures; the eigen-axes of $\Sigma$ have variances $e^{-2r}$ along $\mathbf v$
and $e^{+2r}$ along $\mathbf u$, as expected for a squeezed state. Additionally, if $r=0$, the correlated sideband contribution vanishes and only
the isotropic vacuum fluctuation remains, yielding $\Sigma=\mathbf 1$: the vacuum state.\\

We picture both the $r=0$ case (coherent state) and the $r>0$ case (squeezed state) in Fig. \ref{fig:clicks}. In the vacuum case, the sideband contributions are uncorrelated (no arrows), and the fluctuations are isotropic and of variance 1, yielding a circular distribution in phase space (Fig. \ref{fig:clicks}(a.i)). Individual measurements of the quadrature vector $\mathbf{u}$ yield different outcomes (Fig. \ref{fig:clicks}(a.ii)), such that we recover the usual coherent state representation as seen in Fig. \ref{fig:clicks}(b.i)-(b.iii) (and similarly to the unmodulated sideband modes in Fig. \ref{fig:QSD_mod_phase}). For squeezed states, we follow the \textit{vector-sum of correlated random variables} prescription above, and decompose the conjugate sideband interference into a correlated $2\sinh 2r$ part (arrows) and an isotropic sub-Gaussian $e^{-2r}$ fluctuation part (Fig. \ref{fig:clicks}(c.i) and (c.ii)). Repeated measurements of the quadrature vector $\mathbf{u}$ then yield a elliptical distribution in phase space (Fig. \ref{fig:clicks}(c.iii)), with a reduced variance along the squeezed quadrature $\mathbf v$ and an increased variance along the anti-squeezed quadrature $\mathbf u$. The arrows are correlated such that they always mirror each other with respect to a correlation axis set by the phase of the pump, seen as a green line in Fig. \ref{fig:clicks}(c)-(d). \\ 
...
\begin{figure}[t!]
\centering
\includegraphics[width=\textwidth]{./chap2/fig/clickss.pdf}
\caption[Graphical representation of the quadrature statistics of a squeezed state using the vector-sum picture.]{
(a) Vacuum state: the sidebands are uncorrelated (no arrows), and the field exhibits isotropic vacuum fluctuations with variance $1$. Repeated measurements at frequency $\Omega_{\mathrm{mod}}$ yield different quadrature outcomes (a.ii). (b) Coherent state: the fluctuation part of each sideband remains in the vacuum state, recovering the usual representation of an unmodulated coherent state. (c.i) Squeezed state: the sideband interference decomposes into a correlated contributionn (arrow) and an orthogonal isotropic vacuum contribution. The resulting interference exhibits anti-squeezed fluctuations along the correlated axis and squeezed fluctuations in the orthogonal quadrature (c.ii--c.iii). (d) Quantum sideband picture of a squeezed state, where individual sideband photons are drawn from thermal distributions. Their interference produces correlated quadrature fluctuations, yielding squeezing along one quadrature and anti-squeezing along the orthogonal one.. }
\label{fig:clicks}
\end{figure}
\newpage
...
\begin{figure}[h!]
\centering
\includegraphics[width=\textwidth]{./chap2/fig/QSD_sqz3.pdf}
\caption{Quantum sideband diagram of a squeezed state. Contrary to a modulated field, the sideband photons are drawn from a thermal distribution i.e. they \textit{click} at random quadrature angles. However, as seen in Fig. \ref{fig:clicks}, these photons are correlated such that their sum results in a squeezed state with reduced fluctuations in one quadrature and increased fluctuations in the orthogonal quadrature. This happens at all sideband frequencies within a frequency band determined by the squeezing source, with a frequency-independent angle provided by the pump phase. The three columns correspond to phase squeezing (a), arbitrary angle squeezing (b), and amplitude squeezing (c).} 
\label{fig:QSD_sqz}
\end{figure}
...
\noindent \textbf{Planar travelling-wave cavities:} We now consider a triangular cavity formed by two concave mirrors of radius of curvature $R$ and one flat mirror  (Fig. \ref{fig:cavity_types}.(c)). The stability condition reads
\begin{equation}
...
Considering now a bow-tie cavity formed by two concave mirrors of radius of curvature $R$ and two flat mirrors  (Fig. \ref{fig:cavity_types}.(d)), the full stability condition reads
\begin{equation}
...
If the cavity is stable, it will then feature a discrete set of resonant modes, each corresponding to a cavity length that is an integer multiple of half the wavelength $\lambda/2$ (standing-wave cavity) or the wavelength $\lambda$ (travelling-wave cavity). In the frequency domain, modes are spaced by the free spectral range $\omega_{\mathrm{FSR}}$ of the cavity, defined as
\begin{equation}
  \omega_{\mathrm{FSR}} = \frac{\pi c}{L} \quad \text{(linear cavity)}, \quad \omega_{\mathrm{FSR}} = \frac{2 \pi c}{L_{rt}} \quad \text{(travelling-wave cavity)}
\end{equation}
such that the resonance frequencies are given by
\begin{equation}
  \omega_N = N \, \omega_{\mathrm{FSR}}, \quad N \in \mathbb{N}
\end{equation}
and the cavity is on resonance when the input laser frequency $\omega_0$ matches one of the resonance frequencies $\omega_N$ i.e. $\omega_0 = \omega_N$. To achieve this, one can either tune the laser frequency or the cavity length. In our experiments, we use the second option by mounting one of the cavity mirrors on a piezoelectric actuator. Changing the cavity length $L$ by $\delta L$ shifts the resonance frequencies by
\begin{equation}
...
A cavity also supports TEM$_{mn}$ transverse modes, each with a specific spatial profile and resonant frequency. The resonant frequencies of these transverse modes are shifted relative to the fundamental mode by an amount that depends on the cavity geometry and the mode indices $(m,n)$. Coupling an incoming beam into a stable optical cavity requires that the spatial mode of the beam matches that of the cavity. This means that the mode function of the incoming beam, assumed to be a TEM$_{00}$ Gaussian mode
 $f_{0}(\mathbf{r})$, must overlap with the cavity fundamental mode $f_{0}'(\mathbf{r})$. If the basis functions are not perfectly matched, the incoming field can be expanded in the orthonormal basis of cavity modes as
\begin{equation}
...
\end{split}
\end{equation}
where  \(\kappa = \kappa_0 + \kappa_1 + \kappa_2\) is the total decay rate, with $\kappa_0=\mathcal{L}/\tau$ and $ \delta \hat{a}_{\mathrm{l}}(t)$ the rate and fluctuation operator of additional losses. Here losses $\mathcal{L}$ are unitless (usually given in ppm - parts per million). Another key element to deriving both steady-state behaviour as well as quadrature spectra is the input-output formula given by \cite{gardiner}:
\begin{equation}
...
(c) Low-pass and high-pass transfer functions of the input classical noises as in \eqref{eq:MCIR}}. 
...
\end{equation}
The cavity linewidth (full width at half maximum, or FWHM) is then given by $\kappa/2\pi$ (Hz), as illustrated In Fig. \ref{fig:CavitySimple}.(b). We will then refer to $\kappa/4\pi$ (half width half maximum, or HWHM) as the cavity bandwidth (Hz). Plugging back the expression of $\kappa_i = T_i/\tau$ in the reflection coefficient, we have 
\begin{equation}
  R(|\Delta| \gg \kappa) = 1  \qquad R(\Delta =0) = \Bigg(\frac{T_1 - T_2 - \mathcal{L}}{T_1 + T_2 + \mathcal{L}}\Bigg)^2
\end{equation}
such that the relative depth of the resonance dip gives us information about the cavity losses and couplings. In particular, the resonance dip vanishes when $T_1 = T_2 + \mathcal{L}$, which is the so-called \textit{impedance matching} condition: no light is reflected at resonance and all of it is either transmitted or lost. Similarly, the transmission coefficient reads
\begin{equation}
  T(|\Delta| \gg \kappa) = 0  \qquad T(\Delta =0) = \frac{4 T_1 T_2}{(T_1 + T_2 + \mathcal{L})^2}.
\end{equation}

We also define the cavity finesse $\mathcal{F}$, which is a measure of the sharpness of the cavity resonance peaks relative to its FSR, as
\begin{equation}
...
We now let the detuning vary linearly in time, and express it in units of cavity bandwidth as $\Delta(t)= \Delta_0 + v \frac{\kappa^2}{2}t$ where we define $v$ as the sweep speed in units of cavity bandwidth per $\kappa^{-1}$. The intracavity field yields the useful differential equation
\begin{equation}
...
\end{equation}
We provide the derivatives of the phase terms as they are used later to \textit{phase-match} the filter cavity to the optomechanical cavity, in order to compute the optimal filter parameters. The transfer matrix is then expressed as 
\begin{equation}
\begin{split}
...
\end{equation*}
Here, the cavity acts as an ideal frequency-dependent phase shifter, rotating the quadratures by an angle $- \bar \phi$ and adding an overall phase shift $\delta \phi$ that vanishes when computing noise spectra. Explicitly, the two-photon average phase shift and its derivative are given by 
\begin{equation}
...
Let us consider a configuration such that $\kappa_1 = \kappa_2 \approx \kappa/2$, and where we assume negligible losses $\kappa_0 \ll \kappa_{1,2}$. It represents a cavity where the input and output mirror transmittivities are equal, and we set the laser resonant to the cavity ($\Delta=0$), such that the transmitted spectra in Eq. (\ref{eq:MCIR}) reduces to
\begin{equation}
...
\caption{ Cavity diagram for the Second Harmonic Generation. $\hat a$'s are the various fields at play, $\kappa$'s are the various couplings involved, with $\delta \hat a$'s the associated fluctuations, similar as in Fig. I.4, now considering both the infrared pump, and the generated green beam. The difference in transverse size has been exagerated for clarity.  } 
\end{figure}
...
(a) Gain of the IR seed as a function of the green pump phase for a fixed normalised pump parameter $x=0.8$. The color variations correspond to the pump phase. 
(b) Amplification-deamplification of an IR seed as a function of the squared pump parameter $x^2$. The square is equivalent to considering the pump power ratio $P/P_{\mathrm{thr}} = |B/B_{\mathrm{thr}}|^2$ rather than the amplitude ratio. The colors correspond to the ones on figure (a) (its extremas). The dashed grey line represents the unit gain i.e. without non-linear medium ($x=0$). } 
\end{figure}
...
Note that a genuine \textit{frequency-dependent} squeezing angle could be obtained by detuning the OPO cavity, but the frequency range over which the squeezing angle varies is related to the cavity bandwidth, which is typically large compared to the frequency range of interest in our experiment. This phenomenon was realised experimentally a few years ago \cite{detunedOPOsqz}, but is not the focus of our work. \\
...
\noindent \textbf{The perfect squeezer: } Starting from Eq. \eqref{eq:II85}, in the idealized limit of perfect escape efficiency ($\eta_{\rm esc}=1$) and for analysis frequencies much smaller than the cavity bandwidth ($\Omega/\kappa \ll 1$), the expression simplifies to
...
\noindent\textbf{Frequency dependence: } Similarly to what was seen earlier considering general quantum states, squeezing at an arbitrary angle $\theta$ can be obtained by rotating the covariance matrix. However, one can now make the squeezing angle frequency-dependent above as
\begin{equation}
...
with 
\begin{equation*}  
  \quad \text{with} \quad \phi_m[\Omega] = \arctan\left(\frac{\Gamma_m \Omega}{\Omega_m^2 - \Omega^2}\right) \quad \text{and} \quad |\chi[\Omega]| = \frac{1}{m\sqrt{(\Omega_m^2 - \Omega^2)^2 + (\Gamma_m \Omega)^2}}.
\end{equation*}
...
\begin{equation}
  ...
  \right.
\end{equation}
where we have introduced the radiation-pressure induced detuning $\bar\Delta = \Delta - G x$ (the mean resonator displacement shifts the cavity resonance frequency, hence the detuning), and where we have assumed the input field to be real. \\ 
...
\delta\hat{\mathbf u}_{\mathrm{in}}.
\end{equation}
In GW papers, the prefactor will often be 8 (and not 32) as they use the cavity halfwidth at half maximum rather than $\kappa$. We indeed recover the standard expression used in the GW community, which is a good sanity check of our derivation. We do stress however that this expression is only valid for a free mass, and that the full expression including the mechanical resonant behaviour is required to describe optomechanical cavities in general. \\
...
\begin{equation}
      ...
      \end{pmatrix}
\end{equation}
where the off-diagonal terms are complex conjugates of each other, ensuring the covariance matrix is Hermitian as required. The diagonal terms are the amplitude and phase noise spectra respectively, while the off-diagonal terms quantify correlations between amplitude and phase. The presence of these correlations is the hallmark of optomechanical/ponderomotive squeezing i.e. using the effective non-linear response of the resonator to squeeze light \cite{Aggarwal2020}. This effect is not seen neither sought in our experiment, but is a very active field of research in the optomechanics community. \\

One now sees two essential components in the reflected phase spectrum. The first is the direct phase fluctuations, which is simply shot noise seen as $1$. The second is the back-action term $\propto |\mathcal{K}[\Omega]|^2$, which represents the phase fluctuations induced by the resonator motion driven by radiation-pressure fluctuations. 
...
\end{equation}
where we recognise the mean photocurrent term in $2 e \, |\bar \alpha_{\mathrm{LO}}| \, | \bar \alpha | \, \cos \phi_{\mathrm{LO}}$ as in the two-field direct detection case. This slowly varying mean photocurrent can be used to lock the LO phase $\phi_{\mathrm{LO}}$ to a desired value, as previously described, with a piezoelectric actuator and phase modulation/demodulation scheme if needed. The HD photocurrent fluctuations in Fourier space are then given by
\begin{equation}
...
The practical implementation of these detection schemes and the associated locks are detailed in Chap. III. 
