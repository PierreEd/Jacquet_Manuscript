...
\end{equation}
where we identify a carrier term, a deterministic coherent displacement associated with classical modulations, and an operator fluctuation contribution. Assuming vanishing mean operator fluctuations,
...
\begin{equation}
    ...
    \end{pmatrix},
\end{equation}
while the operator contribution $\mathbf S_{\mathrm{op}}[\Omega]$ is defined as in Eq. \ref{eq:Sa}. 

\subsubsection{Example: Coherent state modulated in amplitude}

For a coherent state, the operator contribution is simply the vacuum noise,
\begin{equation}
    \mathbf S_{\mathrm{op}}[\Omega]=\mathbf 1,
\end{equation}
and therefore
\begin{equation}
    \mathbf S_{\bar{\varphi}}[\Omega]
    ...
    \mathbf 1.
\end{equation}
If modulated in amplitude, the quadrature fluctuation vector reads
\begin{equation}
    \delta\mathbf{\hat u}_{\bar{\varphi}}[\Omega]
    =
    |\bar{\alpha}|\epsilon_a
    \left(
        \delta(\Omega-\Omega_{\mathrm{mod}})
        +
        \delta(\Omega+\Omega_{\mathrm{mod}})
    \right)
    \begin{pmatrix}
    ...
    \end{pmatrix},
\end{equation}
such that its spectral covariance matrix reads
\begin{equation}
    \mathbf S_{\bar{\varphi}}[\Omega]
    =
    2|\bar{\alpha}|^2\epsilon_a^2
    \left(
        \delta(\Omega-\Omega_{\mathrm{mod}})
        +
        \delta(\Omega+\Omega_{\mathrm{mod}})
    ...
    \mathbf 1.
\end{equation}
The covariance matrix therefore consists of coherent spectral lines associated with the deterministic modulation, together with a flat vacuum noise background. Importantly, the coherent modulation contribution is positive semidefinite and can only add spectral power. Sub-shot-noise behavior can only arise from the operator covariance matrix $\mathbf S_{\mathrm{op}}[\Omega]$, which encodes quantum correlations between the sidebands.

\subsection{Quantum Sideband Diagram }

We now have all the tools to graphically represent continuous wave (CW) quantum states of light in the frequency domain. The so-called \emph{quantum sideband diagram} is a useful representation to visualize optical quantum states, especially when dealing with modulated fields or squeezed fields \cite{chelkowski2007,chua2013}. The central idea is that each sideband mode $\omega_0 + \Omega$ out of the frequency continuum can be treated as a quantum harmonic oscillator, featuring some level of correlation with its conjugate sideband mode at $\omega_0 - \Omega$, and represented in its own phase space (different from the two-photon quadratures defined above). \\  


In this representation, we plot the carrier frequency \(\omega_0\) at the center, and the sidebands at frequencies \(\omega_0 \pm \Omega\), such that once in the frame rotating at \(\omega_0\), the carrier field is a DC term with an arbitrary phase, while the conjugate sidebands at \(\pm \Omega\) rotate counter-clockwise and clockwise, respectively. Each sideband mode of this sideband continuum is then associated with a quantum state: a coherent state, a thermal state, etc. As seen in Eq. \ref{eq:modulated_field}, the quadrature fluctuations of the field is given by
\begin{equation*}
\delta \mathbf{\hat{\mathbf u}} _{\bar{\varphi}} [\Omega] = 2|\bar{\alpha}|\begin{pmatrix}
...
\end{pmatrix}
\end{equation*}
where we identified both a classical amplitude contribution from the modulation, as well as quantum fluctuation part, such that the noise seen at a given frequency $\Omega$ when measuring the field with a photodetector is the sum of these. In the time domain, this beating process can be split in two parts, the classical amplitudes of the sidebands which simply add up in phase space, and the quantum fluctuations of the sidebands which average out to a noise level that depends on the quantum state of the sidebands and correlation level. \\ 


We will now cover two useful cases:
\begin{itemize}
  \item classicaly modulated coherent states, where the modulation is either in amplitude or in phase, and where the sideband modes at the modulation frequency are displaced in phase space with a specific phase relation to the carrier, while all the other sideband modes rest in the vacuum state. 
  \item squeezed states, where the squeezing process creates pairs of photons in the sideband modes $\hat{a}_\pm$, each drawn from a thermal distribution larger than vacuum and centered around the origin (hence with no preferred phase), but with a specific correlation between the two sidebands that can be decomposed into a correlated displacement along the squeezing axis and an independent isotropic sub-vacuum fluctuation.
\end{itemize} 
An additional key concept is that of optical beating of the conjugate sideband. , as illustrated in Fig. \ref{fig:mod2}(b). This beating process is what allows to extract the power spectral density (PSD) of the signal at frequency $\Omega$ by averaging over time, for instance with a spectrum analyzer, where we illustrate this in Fig. \ref{fig:mod2}(c) and (d). This graphical tool then pictures the connection between double-sided spectra and single-sided spectra, as well as the connection between the quantum state of the sideband modes and the resulting noise at a given frequency. \\

We start with the representation of a coherent state modulated at $\Omega_\mathrm{mod}$, as seen in Fig. \ref{fig:QSD_mod}(a)-(b). As detailed in the previous section, the modulation sidebands are created with a specific phase relation to the carrier depending on the nature of the modulation (amplitude or phase), while all the other sideband modes rest in the vacuum state (as seen at frequencies $\Omega_1$ and $\Omega_2$). Upon time evolution, the signal at the modulation frequency $\Omega_\mathrm{mod}$ can be seen as the vectorial sum of the two sidebands rotating in opposite directions at $\pm \Omega_\mathrm{mod}$ (Fig. \ref{fig:QSD_mod}). When averaging these signals, with a spectrum analyzer for instance, we extract the PSD of the resulting averaged signal.  We show both cases of amplitude and phase modulation in Fig. \ref{}


The \emph{noise properties} of the state are conveniently represented in phase space, and accessed experimentally with a (typically) well-matched homodyne detection scheme, measuring the field quadratures. In practice, experiments probe these noise properties through repeated measurements and averaging. This experimental access to the statistical properties of the quantum state defines the so-called \emph{ball-on-stick} picture of quantum sidebands. The mean value of the field distribution can be zero or non-zero, while its variance is always non-zero. Looking only at the fluctuations part, we can represent the quantum state of light at a given sideband frequency \(\Omega\), as shown in Fig. \ref{fig:ballonstick}(a--c). In the case of a coherent state (Fig. \ref{fig:ballonstick}(a)-b), the distribution is circular and centered at the origin, indicating equal vacuum fluctuations in both quadratures.\\ 

\begin{figure}[t!]
\centering
\includegraphics[width=\textwidth]{./chap2/fig/mod2.pdf}
\caption{Quantum sideband diagram of a modulated coherent state.}
\label{fig:QSD_mod_amp}
\end{figure}
...
\caption{Ball-on-stick representation of quantum sidebands.
...
We start by describing a coherent state modulated at $\Omega_\mathrm{mod}$, as seen in Fig. \ref{fig:QSD_mod}(a)-(b). As detailed in the previous section, the modulation sidebands are created with a specific phase relation to the carrier depending on the nature of the modulation (amplitude or phase), while all the other sideband modes rest in the vacuum state (as seen at frequencies $\Omega_1$ and $\Omega_2$). Upon time evolution, the signal at the modulation frequency $\Omega_\mathrm{mod}$ can be seen as the vectorial sum of the two sidebands rotating in opposite directions at $\pm \Omega_\mathrm{mod}$ (Fig. \ref{fig:QSD_mod}). When averaging these signals, with a spectrum analyzer for instance, we extract the PSD of the resulting averaged signal. \\ 
