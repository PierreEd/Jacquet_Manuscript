...
Present research in quantum optomechanics now focus on broad range of topics, that we can loosely put in four main categories: (I) the study of quantum limits in position measurements; (II) the investigation of the fundamental quantum physics of the mechanical degree of freedom; (III) the development of improved optomechanical sensors; and (IV) applications to quantum information. These topics all take advantage of the versatility of optomechanical systems, than can be coupled to a large spectrum of systems, such as laser and microwave light, electric and magnetic fields, two-level systems (spins, color centers...), acceleration and forces, etc. We show one landmark example of each category in Fig \ref{fig:panorama}, where we highlight that only membrane based resonators were used in these examples. Over the years, the field of quantum optomechanics has thus expanded to a wide variety of platforms and applications, with mechanical resonators spanning over 12 orders of magnitude in mass, from levitated nanoparticles to kg-scale mirrors, as shown in Fig. \ref{fig:resonatorsfamily}.
...
\begin{figure}[t!]
\centering
\includegraphics[width=\textwidth]{./chap1/fig/panorama.pdf}
\caption{Landmark examples of the four main categories of research in quantum optomechanics: (I) the study of quantum limits in position measurements, with the first experimental demonstration of radiation-pressure noise in tabletop optomechanics \cite{regal2013}; (II) the investigation of the fundamental quantum physics of the mechanical degree of freedom, with the setup used to probe mechanical sideband asymmetry close to the membrane ground state \cite{Lee2015}; (III) the development of improved optomechanical sensors, with the demonstration of optomechanically enhanced force microscopes \cite{Halg2021}; and (IV) applications to quantum information, with the demonstration of optical to microwave transduction \cite{Andrews2014}. All these examples involve membrane based resonators, which have become a popular platform for optomechanics due to their versatility and tunability. }
\label{fig:panorama}
\end{figure}
\endgroup 
...
This thesis belongs to the first category of research mentioned above i.e.  the study of quantum limits in position measurements, where we focus on the use and manipulation of squeezed light to improve the measurement sensitivity beyond the SQL. We do so by leveraging two central experimental ingredients:
\begin{itemize}
...
\noindent \textbf{Chapter III} provides an overview of the experimental methods developed throughout this PhD work, with emphasis on digital locking techniques largely implemented within the Python RedPitaya Lockbox (PyRPL) architecture. \\
